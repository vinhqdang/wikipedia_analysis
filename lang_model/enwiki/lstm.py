...
try:
      opts, args = getopt.getopt(sys.argv[1:],"hd:model_size:epoch:lb:es:",["model_size=","epoch=","es=","lstm_size=","dropout=","dynamic=","activation=","rate=","test_ratio="])
...
for opt, arg in opts:
    ...
    elif opt in ["--lstm_size"]:
        lstm_size = arg.split (",")
        lstm_size = [int (x) for x in lstm_size]
    elif opt in ["--dropout"]:
...
# net = tflearn.lstm(net, lstm_size, dropout=0.5)
for i in range (len (lstm_size)):
  if i < len(lstm_size) - 1:
    net = tflearn.lstm(net, lstm_size [i], activation = activation_function, return_seq = True, dropout=dropout_ratio)
  else:
    net = tflearn.lstm(net, lstm_size [i], activation = activation_function, dropout=dropout_ratio)
...
model = tflearn.DNN(net, tensorboard_verbose=1, tensorboard_dir = "logdir/lstm")
